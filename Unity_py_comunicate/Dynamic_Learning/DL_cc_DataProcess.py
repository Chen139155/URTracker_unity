...
def urtracker_data_process(df:pd.DataFrame, DrawDebug=False):
    """在实时线程中对urtracker数据进行预处理"""
    ...
    if DrawDebug:
        ...
        plt.plot(raw_data["ori_gaze_x"], raw_data["ori_gaze_y"])
        plt.xlabel("x")
        plt.ylabel("y")
    ...
    entire_raw_gaze_x = util.low_pass_filter(entire_raw_gaze_x, 3, 20)
    ...
    entire_raw_gaze_y = util.low_pass_filter(entire_raw_gaze_y, 3, 20)
    # ur track 数据
    entire_raw_ur_x = (
        torch.tensor(raw_data["cursor_x"][:-1], dtype=torch.float) - center_point[0]
    )
    entire_raw_ur_x = util.low_pass_filter(entire_raw_ur_x, 2, 20)
    ...
    entire_raw_ur_y = util.low_pass_filter(entire_raw_ur_y, 2, 20)
    raw_data['force_norm'] = np.sqrt(np.array(raw_data['Hex_x'])**2 + np.array(raw_data['Hex_y'])**2)
    force_norm_tensor = torch.tensor(raw_data['force_norm'][:-1], dtype=torch.float) 
    force_norm_tensor = util.low_pass_filter(force_norm_tensor,0.4,20)
    ...
    #PI = torch.cat((torch.tensor([0.0]), point_diff))
    PI = torch.cat((point_diff[:1], point_diff))
    PI = util.low_pass_filter(PI, 0.16, 20)
...
if __name__ == "__main__":
    xk_N_AM = get_urtracker_data(
        r"./output/cc_NORM_02.xlsx", type="Norm_AM", Debug=False
    )
    xk_ABN_AM = get_urtracker_data(
        r"./output/cc_ABN_A_01.xlsx", type="ABN_A", Debug=False
    )
    ...
    xk_ABNA_gaze = torch.cat(
        [
            xk_ABN_AM["RT_gaze"].unsqueeze(0),
            xk_ABN_AM["TE_gaze"].unsqueeze(0),
            xk_ABN_AM["PI_gaze"].unsqueeze(0),
        ],
        dim=0,
    )
    print("xk_gaze shape:", xk_ABNA_gaze.shape)
    
    xk_ABNA_motion = torch.cat(
        [
            xk_ABN_AM["RT_ur"].unsqueeze(0),
            xk_ABN_AM["TE_ur"].unsqueeze(0),
            xk_ABN_AM["force_norm"].unsqueeze(0),
        ]
    )
    print("xk_motion shape:", xk_ABNA_motion.shape)
    ...
    if draw_debug:
        ...
        plt.plot(xk_N_gaze[0, :].cpu(), label="x")
        plt.plot(xk_N_gaze[1, :].cpu(), label="y")
        plt.plot(xk_N_gaze[2, :].cpu(), label="PI")
        
        plt.legend()
        plt.savefig(os.path.join(path_figsave,"filter_x_y_NORM_gaze" ))
        plt.show()
        ...
        ax.legend()
        plt.savefig(os.path.join(path_figsave,"filter_x_y_NORM_gaze_3d" ))
        plt.show()
        ...
        plt.legend()
        plt.savefig(os.path.join(path_figsave,"filter_x_y_NORM_motion" ))
        plt.show()
        ...
        ax.plot(x, y, z, label='parametric curve')
        ax.legend()
        plt.savefig(os.path.join(path_figsave,"filter_x_y_NORM_motion_3d" ))
        plt.show()
        '''xk_ABNA'''
        plt.figure("xk_ABNA_gaze")
        plt.plot(xk_ABNA_gaze[0, :].cpu(), label="x")
        plt.plot(xk_ABNA_gaze[1, :].cpu(), label="y")
        plt.plot(xk_ABNA_gaze[2, :].cpu(), label="PI")
        
        plt.legend()
        plt.savefig(os.path.join(path_figsave,"filter_x_y_ABNA_gaze" ))
        plt.show()

        fig = plt.figure("gaze")
        ax = fig.add_subplot(111, projection='3d')
        z = xk_ABNA_gaze[2, :].cpu()
        x = xk_ABN_AM["raw_data"]['filter_gaze_x']
        y = xk_ABN_AM["raw_data"]['filter_gaze_y']
        ax.plot(x, y, z, label='parametric curve')
        ax.legend()
        plt.savefig(os.path.join(path_figsave,"filter_x_y_ABNA_gaze_3d" ))
        plt.show()

        plt.figure("xk_N_motion")
        plt.plot(xk_ABNA_motion[0, :show_length].cpu(), label="RT_ur")
        plt.plot(xk_ABNA_motion[1, :show_length].cpu(), label="TE_ur")
        plt.plot(xk_ABNA_motion[2, :show_length].cpu(), label="force")
        plt.legend()
        plt.savefig(os.path.join(path_figsave,"filter_x_y_ABAN_motion" ))
        plt.show()

        fig = plt.figure("motion")
        ax = fig.add_subplot(111, projection='3d')
        z = xk_ABNA_motion[2, :].cpu()
        x = xk_ABN_AM["raw_data"]['filter_ur_x']
        y = xk_ABN_AM["raw_data"]['filter_ur_y']
        ax.plot(x, y, z, label='parametric curve')
        ax.legend()
        plt.savefig(os.path.join(path_figsave,"filter_x_y_ABAN_motion_3d" ))
        plt.show()

...
from UR5_admittance import AdmittanceForceMode
...
def get_admittance_params_by_assistance_level(assistance_level):
    ...
    if assistance_level == 'high':
        return {'K': np.array([800, 800, 800]), 'D': np.array([225, 225, 225]), 'M': np.array([100, 100, 100])}
    elif assistance_level == 'medium':
        return {'K': np.array([600, 600, 600]), 'D': np.array([225, 225, 225]), 'M': np.array([100, 100, 100])}
    elif assistance_level == 'low':
        return {'K': np.array([400, 400, 400]), 'D': np.array([225, 225, 225]), 'M': np.array([100, 100, 100])}
    else:
        # 默认返回中等辅助参数
        return {'K': np.array([600, 600, 600]), 'D': np.array([225, 225, 225]), 'M': np.array([100, 100, 100])}
...
def zmq_message_receiver(socket, target_queue, task_stage_queue):
    """
    ZMQ消息接收线程函数
    持续接收消息并根据类型放入不同的队列
    """
    while True:
        try:
                ...
                try:
                    ...
                    if "topic" in data:
                        ...
                        if topic == "TargetPosition":
                            # TargetPosition消息只保留最新的
                            if target_queue.full():
                                target_queue.get_nowait()  # 移除旧消息
                            target_queue.put_nowait(data)
                            
                        elif topic == "TaskStage":
                            # TaskStage消息添加到队列
                            task_stage_queue.put_nowait(data)
                            logging.debug(f"TaskStage消息已加入队列: {data['stage']}")
            ...
            # 没有数据可接收，短暂休眠
            time.sleep(0.001)  # 1ms休眠，减少CPU占用
            continue
...
if __name__ == "__main__":
    ...
    # target_sub_socket.setsockopt(zmq.CONFLATE, 1)  # 移除这一行
    target_sub_socket.setsockopt(zmq.RCVHWM, 50)    # 增加接收高水位，确保能存储足够的消息
    ...
    # 创建消息队列用于存储不同类型的消息
    target_queue = queue.Queue(maxsize=1)  # 只保留最新的TargetPosition消息
    task_stage_queue = queue.Queue(maxsize=10)  # 可以存储多个TaskStage消息（避免覆盖）
    # 启动ZMQ消息接收线程
    zmq_thread = threading.Thread(target=zmq_message_receiver, 
                                args=(target_sub_socket, target_queue, task_stage_queue), 
                                daemon=True)
    zmq_thread.start()
    # 初始化UR5控制器
    g2r_q, r2g_q, cmd_q = queue.Queue(maxsize=10), queue.Queue(maxsize=10), queue.Queue(maxsize=10)
    ur5 = AdmittanceForceMode(
        adm_M=current_M,
        adm_D=current_D,
        adm_K=current_K
    )
    # ur5_thread = threading.Thread(target=ur5.start, args=(g2r_q, r2g_q, cmd_q), daemon=True)
    # ur5_thread.start()
    ur5.start(g2r_q, r2g_q, cmd_q)
                ...
                if not target_queue.empty():
                    try:
                        ...
                        if "x" in target_data and "y" in target_data:
                            target = {"x": target_data["x"], "y": target_data["y"]}
                            target_r = pos_g2r(r_r, r_g, pos_cg, pos_cr, [target["x"], target["y"]])
                    except Exception as e:
                        logging.error(f"处理TargetPosition消息时出错: {e}")

                # 处理TaskStage消息
                if not task_stage_queue.empty():
                    try:
                        while not task_stage_queue.empty():
                            # 处理所有排队的TaskStage消息（确保不遗漏）
                            task_stage_data = task_stage_queue.get_nowait()
                            if "stage" in task_stage_data:
                                task_stage = task_stage_data
                        ...
                        logging.error(f"处理TaskStage消息时出错: {e}")
